...
import time
import numpy as np
from store_exercise import store_exercise_data
...
def start_bridging_exercise_monitor(video_path=None, user_id=None):
    counter = 0
    stage = "down"
    user_side = None
    reps_data = []
    ...
    while cap.isOpened():
        ...
        try:
            ...
            if stage == 'up' and normalized_hip_raise < 0.1:
                time_in_up_state = time.time() - up_start_time
                reps_data.append(
                    {"rep": counter, "time_in_up": time_in_up_state})
            ...
            cv2.putText(
                image,
                f'Time in Up State: {time_in_up_state:.2f} s',
                (10, 150),  # Position (x, y)
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,  # Font scale
                (0, 0, 0),  # Color (B, G, R)
    ...
    store_exercise_data(user_id, "Bridging", counter, reps_data)

    cap.release()
    cv2.destroyAllWindows()


def start_leg_raise_monitor(video_path=None, user_id=None):
    counter = 0
    stage = "down"
    user_side = None
    reps_data = []
    ...
    while cap.isOpened():
        ...
        try:
            ...
            angle = calculate_angle(
                [relevant_shoulder[0], relevant_shoulder[1]], [relevant_hip[0], relevant_hip[1]], [relevant_knee[0], relevant_knee[1]])

            # print angle on hip point
            cv2.putText(image, f'{angle:.2f}', (int(relevant_hip[0] * 640), int(relevant_hip[1] * 480)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)

            # Monitor the exercise based on angle
            if stage == 'up' and angle > 120:
                time_in_up_state = time.time() - up_start_time
                reps_data.append(
                    {"rep": counter, "time_in_up": time_in_up_state})
